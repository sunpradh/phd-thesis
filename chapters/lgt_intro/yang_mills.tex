...
\end{equation}
The measures $\DD A$, $\DD \overline{\psi}$ and $\DD \psi$ can be physically interpreted as a measure over all the possible configurations of the fields $A_{\mu}$, $\overline{\psi}$ and $\psi$, respectively, but are still lacking a rigorous mathematical definition, which is a topic we will not elaborate on \cite{peskin1995qft}.
