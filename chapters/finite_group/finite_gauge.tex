...
% In the Hamiltonian formulation of \ac{lgt}s \cite{kogut1975hamiltonian, milstead2018qyangmills, ZoharBurrello}, time is continuous while the $d$ spatial dimensions are discretized into a hypercubic lattice.
...
%in the analogy where $\{\ket{g}\}$ is a position basis in group space \cite{ZoharBurrello},
\begin{equation}
    L_h \ket{g} = \ket{hg}
    \qand
    R_h \ket{g} = \ket*{gh^{-1}}.
...
The Kogut-Susskind Hamiltonian eq.~\eqref{eq:kog suss hE hB} may be obtained via the transfer-matrix formulation from the Euclidean Wilson action \cite{creutz1977transfermatrix,KogRev}
\begin{equation}
