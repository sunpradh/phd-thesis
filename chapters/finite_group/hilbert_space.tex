...
\begin{figure}[t]
    \centering
    \input{assets/figures/lattice_decomp.tex}
    \smallskip
...
\begin{figure}[t]
    \centering
    \SideFigure[label=fig:gauss_op, desc={Gauss operator in the representation basis}]{
        \input{assets/figures/gauss_op.tex}
    }{%
        Gauss operator in the representation basis.
        Notice that it acts on a single block (green region) of the total Hilbert space decomposition \eqref{eq:hilbert_space_site}.
    }
\end{figure}
...
Now let $A = \pqty{a_1, a_2, a_3, a_4}$, which implicitly depends on $\{j\}$ (because the range of each $a_x$ depends on the \acp{irrep} assigned around the site $x$).
...
\begin{align}
    \begin{split}
        \ket{ \{j\};A}
        % = \sum_{n_1, \ldots n_8} \sum_{m_1, \ldots m_8}
        = \sum_{ \{m,n\} }
